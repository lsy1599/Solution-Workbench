...
const MSBUILD_PATH_NAME = 'msbuildPath';
const MSBUILD_CONFIGURATION_NAME = 'msbuildConfiguration';
const MSBUILD_PLATFORM_NAME = 'msbuildPlatform';
const MSBUILD_VERBOSITY_NAME = 'msbuildVerbosity';
const MSBUILD_ADDITIONAL_ARGS_NAME = 'msbuildAdditionalArgs';
const MSBUILD_PARALLEL_BUILD_NAME = 'msbuildParallelBuild';
const MSBUILD_PLATFORM_TOOLSET_NAME = 'msbuildPlatformToolset';
...
export function getMsBuildPath(): string {
    return config.get<string>(MSBUILD_PATH_NAME, "");
}

export function getMsBuildConfiguration(): string {
    return config.get<string>(MSBUILD_CONFIGURATION_NAME, "");
}

export function getMsBuildPlatform(): string {
    return config.get<string>(MSBUILD_PLATFORM_NAME, "");
}

export function getMsBuildVerbosity(): string {
    return config.get<string>(MSBUILD_VERBOSITY_NAME, "minimal");
}

export function getMsBuildAdditionalArgs(): string[] {
    return config.get<string[]>(MSBUILD_ADDITIONAL_ARGS_NAME, []);
}

export function getMsBuildParallelBuild(): boolean {
    return config.get<boolean>(MSBUILD_PARALLEL_BUILD_NAME, true);
}

export function getMsBuildPlatformToolset(): string {
    return config.get<string>(MSBUILD_PLATFORM_TOOLSET_NAME, "");
}
...
export function getProjectDebugSettings(projectPath: string): ProjectDebugSettings {
    const allSettings = config.get<{ [key: string]: ProjectDebugSettings }>("projectDebugSettings", {});
    const key = projectPath.replace(/\\/g, "/").toLowerCase();
    return allSettings[key] || { commandArgs: "", workingDirectory: "" };
}

export async function setProjectDebugSettings(projectPath: string, settings: ProjectDebugSettings): Promise<void> {
    const allSettings = config.get<{ [key: string]: ProjectDebugSettings }>("projectDebugSettings", {});
    const key = projectPath.replace(/\\/g, "/").toLowerCase();
    allSettings[key] = settings;
    await vscode.workspace.getConfiguration(CONFIGURATION_NAME).update("projectDebugSettings", allSettings, vscode.ConfigurationTarget.Workspace);
}
