...
export class DeleteUnifiedCommand extends ActionsCommand {
    ...
    private prepareDeleteActionGetters(showDialog: boolean) {
        ...
        return prepareContextActionGetters([
            ...
            [ContextValues.project, item => {
                if (!item.project) { return []; }
                const isCps = item.contextValue?.endsWith('-cps');
                if (isCps) {
                    return [new DotNetRemoveExistingProject(item.solution.fullPath, item.project.fullPath)];
                }
                return [new SlnRemoveProject(item.solution.fullPath, item.project.fullPath)];
            }],
