...
export class SlnRenameProject extends SlnRenameSolutionFolder {
    ...
    public async execute(context: ActionContext): Promise<void> {
        ...
        let safeName = this.newprojectname;
        if (safeName.endsWith(extension)) {
            safeName = safeName.slice(0, -extension.length);
        }
        const newProjectPath = path.join(path.dirname(this.project.fullPath), safeName) + extension;
